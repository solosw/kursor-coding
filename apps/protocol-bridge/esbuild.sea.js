 ...
 */
const esbuild = require("../../node_modules/esbuild") // Use root esbuild (0.27+)
...
const swaggerStubPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@nestjs\/swagger$/ }, () => ({
      path: path.join(__dirname, "src", "sea", "swagger-stub.js"),
    }))
  },
}
...
const tiktokenWasmPlugin = {
  ...
  setup(build) {
    ...
    const rootNodeModules = path.join(__dirname, "..", "..")
...
async function build() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, "src", "sea-entry.ts")],
    bundle: true,
    platform: "node",
    target: "node24",
    format: "cjs",
