...
async function bootstrap() {
  ...
  const directApiConfig = app.get(DirectApiConfigService)
  const isDirectMode = directApiConfig.isDirectMode()
  ...
  // Build direct mode banner
  const buildDirectModeBanner = () => {
    const activeEntries = directApiConfig.getActiveEntries()
    const configPath = directApiConfig.getConfigPath() || "not found"

    const modeLabel = `${c.purple}${c.bold}DIRECT${c.reset}`

    const modelLines = activeEntries.map((entry) => {
      const formatIcon =
        entry.format === "anthropic"
          ? `${c.blue}A${c.reset}`
          : `${c.green}O${c.reset}`
      return line(
        `  ${formatIcon} ${entry.customModelId}  ${c.dim}→${c.reset}  ${entry.name || entry.customModelId}`
      )
    })
...
${c.blue}║${c.reset}${pad("", Math.floor((W - 34) / 2))}${c.bold}${c.purple}⚡ Agent Vibes Proxy Server ⚡${c.reset}${pad("", Math.ceil((W - 34) / 2))}${c.blue}║${c.reset}
${sep}
${empty}
...
${line(`${c.green}▸${c.reset} Mode      ${modeLabel}`)}
${empty}
${sep}
${line(`${c.purple}${c.bold}Direct API Configuration${c.reset}`)}
${line(`  ${c.dim}Config: ${configPath}${c.reset}`)}
${empty}
${line(`${c.yellow}${c.bold}Available Models${c.reset}`)}
${modelLines.length > 0 ? modelLines.join("\n") : line(`  ${c.dim}No models configured${c.reset}`)}
${empty}
${sep}
...
`
  ...
  // Build standard mode banner
  const buildStandardModeBanner = () => {
    return `
${logo.join("\n")}
...
`
  ...
  console.log(
    isDirectMode ? buildDirectModeBanner() : buildStandardModeBanner()
  )
}
