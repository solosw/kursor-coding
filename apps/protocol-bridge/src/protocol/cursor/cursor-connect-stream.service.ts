...
export class CursorConnectStreamService {
  ...
  private async *executeBackendStreamWithFallback(
    ...
    try {
      if (this.modelRouter.isDirectMode) {
        const directEntry = this.directApiConfig.findEntryByCustomModel(
          dto.model
        )
        if (!directEntry) {
          throw new Error(
        ...
        if (route.backend === "claude-api") {
          this.logger.log(
            `[DIRECT] Routing to Claude API backend for model: ${route.model}`
          )
          for await (const event of this.claudeApiService.sendDirectClaudeMessageStream(
            routedDto,
            {
  ...
  private getBackendContextLimit(
    ...
    if (this.modelRouter.isDirectMode && model) {
      const directEntry = this.directApiConfig.findEntryByCustomModel(model)
    ...
    if (backend === "openai-compat" || backend === "codex") {
