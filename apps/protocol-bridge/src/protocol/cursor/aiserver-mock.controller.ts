...
export class AiserverMockController {
  ...
  private isCursorModelCurrentlyRoutable(modelId: string): boolean {
    if (this.modelRouter.isDirectMode) {
      return this.directApiConfig.findEntryByCustomModel(modelId) != null
    }
  ...
  private buildCursorModels(options?: { excludeMaxNamedModels?: boolean }) {
    if (this.modelRouter.isDirectMode) {
      return this.directApiConfig
        .getCursorDisplayModels()
        .filter((model) =>
          options?.excludeMaxNamedModels ? !model.name.includes("max") : true
        )
    }
    ...
    try {
      ...
      const protoModels = allModels.map((m) =>
        create(AvailableModelsResponse_AvailableModelSchema, {
          ...
          contextTokenLimit: m.contextTokenLimit || (m.family === "gemini" ? 1000000 : 200000),
          contextTokenLimitForMaxMode: m.contextTokenLimit || (m.family === "gemini" ? 1000000 : 200000),
  ...
  handleCppAvailableModels(@Res() res: FastifyReply): void {
    ...
    const response = create(AvailableCppModelsResponseSchema, {
      models,
      defaultModel: models[0] || DEFAULT_GEMINI_MODEL,
    })
  ...
  @Post("v1/traces")
  handleTraces(@Req() _req: FastifyRequest, @Res() res: FastifyReply): void {
