...
export interface DirectRouteMatch {
  backend: Extract<BackendType, "openai-compat" | "claude-api" | "codex">
...
export class ModelRouterService {
  ...
  get isDirectMode(): boolean {
    return this.directApiConfig.isDirectMode()
  }

  resolveDirectRouteMatch(modelId: string): DirectRouteMatch | null {
    const entry = this.directApiConfig.findEntryByCustomModel(modelId)
  ...
  resolveModel(cursorModel: string): ModelRouteResult {
    ...
    if (this.directApiConfig.isDirectMode()) {
      const directMatch = this.resolveDirectRouteMatch(normalized)
      if (!directMatch) {
        throw new Error(
          `Direct mode: model ${cursorModel} is not defined in apis.yaml.`
        )
      }

      return {
        backend: directMatch.backend,
        model: directMatch.targetModelId,
        isThinking: doesModelSupportThinking(directMatch.targetModelId),
      }
    }
