...
当你想让代理忽略旧的账号池，仅通过专用 YAML 文件路由模型时，可以使用直连模式。
...
- 只有 `active: true` 的条目才会被暴露。
- 请求中的模型名会与 `custom_model_id` 进行匹配。
- `format: anthropic` 的条目会通过 Anthropic Messages API 转发到 `endpoint`。
- `format: openai` 的条目会通过 OpenAI 兼容 API 转发到 `endpoint`。
- 上游请求使用 `target_model_id` 作为真实的目标模型名称。
- 在直连模式下，代理以该 YAML 文件为唯一事实来源，不再依赖旧的账号 JSON 文件。
