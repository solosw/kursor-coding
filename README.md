...
做了什么？
不依赖codex claudecode等配置工具，而是新增直连模式，可以直接定义模型，不用读取配置文件。 模拟cursor操作环境，不依赖账号，使用更加通用的管理界面electron，不依赖插件
...
```bash
# Download
curl -L -o agent-vibes-darwin-arm64-0.1.3.vsix https://github.com/funny-vibes/agent-vibes/releases/download/v0.1.3/agent-vibes-darwin-arm64-0.1.3.vsix

# Install
cursor --install-extension agent-vibes-darwin-arm64-0.1.3.vsix --force
```
...
```bash
# Download
curl -L -o agent-vibes-darwin-x64-0.1.3.vsix https://github.com/funny-vibes/agent-vibes/releases/download/v0.1.3/agent-vibes-darwin-x64-0.1.3.vsix

# Install
cursor --install-extension agent-vibes-darwin-x64-0.1.3.vsix --force
```
...
```bash
# Download
curl -L -o agent-vibes-linux-x64-0.1.3.vsix https://github.com/funny-vibes/agent-vibes/releases/download/v0.1.3/agent-vibes-linux-x64-0.1.3.vsix

# Install
cursor --install-extension agent-vibes-linux-x64-0.1.3.vsix --force
```
...
```powershell
# Download
Invoke-WebRequest -Uri "https://github.com/funny-vibes/agent-vibes/releases/download/v0.1.3/agent-vibes-win32-x64-0.1.3.vsix" -OutFile "agent-vibes-win32-x64-0.1.3.vsix"

# Install
cursor --install-extension agent-vibes-win32-x64-0.1.3.vsix --force
```
...
Use direct mode when you want the bridge to ignore the legacy account pools and route models only from a dedicated YAML file.

Start the bridge in direct mode:
...
```bash
agent-vibes --mode direct --apis-config /absolute/path/to/apis.yaml
```

If `--apis-config` is omitted, the bridge looks for:

- `~/.agent-vibes/data/apis.yaml`
- `~/.agent-vibes/data/apis.yml`
...
apis:
  - name: claude-sonnet
    ...
    custom_model_id: claude-sonnet-4-5
    target_model_id: claude-sonnet-4-5
    custom_api_key: "replace-with-your-anthropic-key"
    active: true
  ...
  - name: gpt-5.4
    ...
    custom_api_key: "replace-with-your-openai-key"
    active: true
```

Behavior:

- Only `active: true` entries are exposed.
- Incoming model names are matched against `custom_model_id`.
- Anthropic-format entries forward to `endpoint` using the Anthropic Messages API.
- OpenAI-format entries forward to `endpoint` using the OpenAI-compatible API.
- The upstream request uses `target_model_id` as the real provider model name.
- In direct mode, the bridge should be treated as using this YAML as the source of truth instead of the legacy account JSON files.
